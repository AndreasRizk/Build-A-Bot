...
if (exists(f"{data_path}wordle_games")): # Reads in saved wordle games on launch if they exist
    with open(f"{data_path}wordle_games","rb") as f:
        games = pickle.load(f)      

def save(): # Save funtion to call when games dictionary is updated
    with open(f"{data_path}wordle_games","wb") as f:
        pickle.dump(games,f)


words = {} # Dictionary to hold all possible words

if (exists(f"extensions/data/wordle_words")): # If the dictionary of words is saved load it in
    with open(f"extensions/data/wordle_words","rb") as f:
        words = pickle.load(f)      
else: # Otherwise create the dictionary from the txt and save it
    with open('words.txt') as f:
        for i in f:
            words[i.strip()] = 0
    with open(f"extensions/data/wordle_words","wb") as f:
        pickle.dump(words,f)
